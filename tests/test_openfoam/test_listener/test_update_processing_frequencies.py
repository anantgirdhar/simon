...
def test_raises_value_error_when_updated_together_and_keep_every_is_larger_than_compress_every(
    keep_every: Decimal, compress_every: Decimal, listener: OFListener
) -> None:
    with pytest.raises(ValueError):
...
def test_raises_value_error_when_updated_together_and_keep_every_is_equal_to_compress_every(
    keep_every: Decimal,
    compress_every: Decimal,
    listener: OFListener,
) -> None:
...
def test_updates_values_together_successfully_when_they_are_a_valid_pair(
    keep_every: Decimal, compress_every: Decimal, listener: OFListener
) -> None:
    listener.update_processing_frequencies(
        keep_every=keep_every, compress_every=compress_every
...
def test_raises_value_error_when_updated_together_and_keep_every_does_not_divide_compress_every(
    keep_every: Decimal, compress_every: Decimal, listener: OFListener
) -> None:
    with pytest.raises(ValueError):
        listener.update_processing_frequencies(
